...
inputData = '''turn on 887,9 through 959,629
...
def instruct(instruction):
    ...
    if instruction.startswith("toggle"):
        ...
        action = "turn on"
    instruction = instruction.split(action+" ")[1]
    param1, param2 = instruction.split(" through ")
    param1, param2 = ( (int(param1.split(",")[0]), int(param1.split(",")[1])), (int(param2.split(",")[0]), int(param2.split(",")[1])) )
    for x in range(param1[0], param2[0]+1):
        for y in range(param1[1], param2[1]+1):
            if action == "toggle":
                lights[x][y] += 2
...
print("The total brightness is "+addComma(totalBrightness))