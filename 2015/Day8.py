inputData = '''"azlgxdbljwygyttzkfwuxv"
...
for line in newLines:
    charactersInCode = len(line)
    stringInMemory = line.replace('\"', ".").replace('\\', ".")
    for index in range(len(stringInMemory)):
        try:
            if stringInMemory[index] == "\\":
                count += 1
            elif stringInMemory[index+1]== "x":
                count += 1
                index += 3
        except:
            pass
